...
class Fs2Match2
  ...
  def initialize(search_obj, result_obj, baseline)
    ...
    i = 0
    while search_obj[i] do
      @search_matrix[i] = Array.new(3)
    ...
    i = 0
    while result_obj[i] do
      @result_matrix[i] = Array.new(3)
  ...
  def match
    ...
    while @search_matrix[i_search] do
      
      i_result = 0
      
      while @result_matrix[i_result] do
        
        # Match skill name (column = 0)
        match_keyword(i, [i_search, 0], [i_result, 0])
        
        # if match_id_exists?(i)
          # Match skill years experience
          # match_integer(i, [i_search, 1], [i_result, 1])
          
          # Match skill details
          # match_keyword_array(i, [i_search, 2], [i_result, 2])
        # end
    ...
    if @matched_cells
      @matched_cells.each_value do |matched_cell|
        
        if (0..4).include?(matched_cell[:skill_match][:search_cell][0].to_i) && (0..4).include?(matched_cell[:skill_match][:result_cell][0].to_i)
          if (0..1).include?(matched_cell[:skill_match][:search_cell][0].to_i) && (0..1).include?(matched_cell[:skill_match][:result_cell][0].to_i)
            @pct += 25
          elsif ((0..1).include?(matched_cell[:skill_match][:search_cell][0].to_i) && (2..4).include?(matched_cell[:skill_match][:result_cell][0].to_i)) ||
            ((2..4).include?(matched_cell[:skill_match][:search_cell][0].to_i) && (0..1).include?(matched_cell[:skill_match][:result_cell][0].to_i))
            @pct += 20
          elsif (2..4).include?(matched_cell[:skill_match][:search_cell][0].to_i) && (2..4).include?(matched_cell[:skill_match][:result_cell][0].to_i)
            @pct += 15
          end
        elsif ((0..4).include?(matched_cell[:skill_match][:search_cell][0].to_i) && (5..9).include?(matched_cell[:skill_match][:result_cell][0].to_i)) ||
            ((5..9).include?(matched_cell[:skill_match][:search_cell][0].to_i) && (0..4).include?(matched_cell[:skill_match][:result_cell][0].to_i))  
          @pct += 10
        elsif ((0..4).include?(matched_cell[:skill_match][:search_cell][0].to_i) && matched_cell[:skill_match][:result_cell][0].to_i > 9) ||
            ((matched_cell[:skill_match][:search_cell][0].to_i > 9) && (0..4).include?(matched_cell[:skill_match][:result_cell][0].to_i))
          @pct += 5
        else
          @pct += 2
        end
