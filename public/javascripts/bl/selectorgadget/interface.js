...
// Main method for making the border around the element
SelectorGadget.prototype.makeBorders = function(orig_elem, makeRed) {
  this.removeBorders();
  this.setupBorders();

  // Prepare the text to display in the borders
  var path_to_show = "";
//  var path_to_show = orig_elem.innerHTML;
...
// Method for finding the parent element
SelectorGadget.prototype.firstSelectedOrSuggestedParent = function(elem) {
  var orig = elem;
  var css_selector_s = "";
  
  if (jQuery(elem).hasClass('sg_suggested') || jQuery(elem).hasClass('sg_selected')) {
  	return elem;
  }
  
  // Iterate while the element has a parent element
...
SelectorGadget.prototype.sgMousedown = function(e) {
  ...
  if (elem == document.body || elem == document.body.parentNode) return;
	...
	var	i_selected_elem = elem;
	var t_selected_elem = "";
	var css_selector_s = null;
	var i = 0;
	
	if (g_selector_mode == g_selector_modes_h["admin_selector"]) {
		
		// 'i_selected_elem' is the ITERATOR element. The start of the loop checks the parent of the node
		// 'css_selector_s' is the CSS SELECTOR that is being constructred at each iteration
		// 't_selected_elem' is the TAG NAME attribute
		while (i_selected_elem && i++ < 40) {
			t_selected_elem = i_selected_elem.tagName;
	
			if (i_selected_elem.getAttribute("id") && i_selected_elem.getAttribute("id") != "null") {
				t_selected_elem += "[id='" + i_selected_elem.getAttribute("id") + "']";
			}
			
			if (i_selected_elem.getAttribute("class") && i_selected_elem.getAttribute("class") != "null") {
				t_selected_elem += "[class='" + i_selected_elem.getAttribute("class") + "']";
			}
			
			css_selector_s = !css_selector_s ? t_selected_elem : t_selected_elem + ">" + css_selector_s;
			
			if (t_selected_elem.toLowerCase() == "html" || !i_selected_elem.parentNode) { break; }
			
			i_selected_elem = i_selected_elem.parentNode;
		}
		
		$("#selector_text").html(css_selector_s);
		$("#parse_text").html(css_selector_s);
		testParsing();
			
	} else if (g_selector_mode == g_selector_modes_h["job_selector"]) {
		
		jobSelectorMouseDownEventHandler(i_selected_elem);
		
	} else if (g_selector_mode == g_selector_modes_h["facebook_selector"]) {
		
		facebookMouseDownEventHandler(i_selected_elem);
